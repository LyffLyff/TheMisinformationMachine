...
func _ready():
	...
	# Highlighting fix/workaround
	country_details.connect("mouse_entered", self._on_mouse_exited.bind("", true))
	bottom.connect("mouse_entered", self._on_mouse_exited.bind("", true))
...
func _on_mouse_exited(country_name: String, ui_fix: bool = false) -> void:
	if ui_fix:
	...
	for node in get_tree().get_nodes_in_group(country_name):
		...
		if hover_polygon == polygon:
			hover_polygon = null

		if Global.CURRENT_COUNTRY != country_name:
			# Determine the correct target color
			...
			if idx == 0 and CountryData.has_progression_started(country_name):
				target_color = COUNTRY_PROGRESSION_STARTED
			else:
				target_color = COUNTRY_PROGRESSION_COLORS[idx]

			# Smooth transition back to target color
			get_tree().create_tween().tween_property(
				polygon,
				"color",
				target_color,
